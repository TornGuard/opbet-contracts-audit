...
await opnet('OPBETRaise — full test suite', async (vm: OPNetUnit) => {
    ...
    await vm.it('buyWithBTC: reverts when payment exceeds remaining cap', async () => {
        ...
        await Assert.expect(async () => raise.buyWithBTC(buyer2, buyer2)).toThrow();
        clearTx();
    ...
    await vm.it('buyWithBTC: reverts when presale is paused', async () => {
        ...
        mockTreasuryOutput(1_000n);
        await Assert.expect(async () => raise.buyWithBTC(buyer, buyer)).toThrow();
    });

    await vm.it('buyWithBTC: reverts with zero recipient address', async () => {
        mockTreasuryOutput(1_000n);
        await Assert.expect(async () => raise.buyWithBTC(Address.dead(), buyer)).toThrow();
    });
    ...
    await vm.it('buyWithBTC: reverts when BTC price not set (satsPerToken = 0)', async () => {
        ...
        mockTreasuryOutput(1_000n);
        await Assert.expect(async () => raise0.buyWithBTC(buyer, buyer)).toThrow();
    ...
    await vm.it('setBTCPrice: non-owner reverts', async () => {
        await Assert.expect(async () => raise.setBTCPrice(100n, buyer)).toThrow();
    });
    ...
    await vm.it('setPresaleActive: non-owner reverts', async () => {
        await Assert.expect(async () => raise.setPresaleActive(false, buyer)).toThrow();
    });
    ...
    await vm.it('setMerkleRoot: non-owner reverts', async () => {
        const { root } = buildTree1(buyer);
        await Assert.expect(async () => raise.setMerkleRoot(root, buyer)).toThrow();
    });
    ...
    await vm.it('claimAirdrop: double-claim reverts', async () => {
        ...
        await Assert.expect(async () => raise.claimAirdrop(buyer, proof, buyer)).toThrow();
    });
    ...
    await vm.it('claimAirdrop: wrong proof reverts', async () => {
        ...
        await Assert.expect(async () => raise.claimAirdrop(buyer, badProof, buyer)).toThrow();
    });
    ...
    await vm.it('claimAirdrop: reverts with proof exceeding 30 levels', async () => {
        ...
        await Assert.expect(async () => raise.claimAirdrop(buyer, hugeProof, buyer)).toThrow();
    });
    ...
    await vm.it('claimAirdrop: reverts with zero recipient', async () => {
        ...
        await Assert.expect(async () => raise.claimAirdrop(Address.dead(), proof, buyer)).toThrow();
    });
    ...
    // Note: deployment-revert tests (e.g. zero treasury key) are enforced at the
    // contract level via onDeployment Revert. The WASM VM catches the revert
    // on-chain but ContractRuntime.init() does not surface it as a JS exception.
    // Security is verified by the presence of the guard in OPBETRaise.ts:
    //   if (u256.eq(treasuryKey, u256.Zero)) throw new Revert('Invalid treasury key');

    await vm.it('deployment: valid key initialises treasury correctly (satsPerToken readable)', async () => {
        const info = await raise.presaleInfo();
        Assert.expect(info.satsPerToken).toEqual(SATS_PER_TOKEN);
        Assert.expect(info.active).toEqual(true);
    });
